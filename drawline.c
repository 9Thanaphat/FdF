...
void draw_line(t_points *points, t_vars *vars)
{
    ...
    int sx = (points->iso_x1 < points->iso_x2) ? 1 : -1;
    int sy = (points->iso_y1 < points->iso_y2) ? 1 : -1;
    int err = dx - dy;
    ...
    while (1) {
        // วาดจุดที่ (x1, y1)
        my_mlx_pixel_put(vars->img_ptr, points->iso_x1, points->iso_y1, ft_gradient(0xff00e6, 0x00f7ff, 256, ft_map(z, vars->grid_ptr->min, vars->grid_ptr->max, 0, 255)));
		...
		int e2 = err * 2;
		if (e2 > -dy) { err -= dy; points->iso_x1 += sx; }
		if (e2 < dx) { err += dx; points->iso_y1 += sy; }
